...
def send_appointment_emails(appt, action="new"):
    subjects = {
        "new": (
            f"[CRETA] Nuevo turno #{appt.id} — {appt.service.name}",
            "Tu turno en CRETA — Solicitud recibida",
        ),
        "confirmed": (
            f"[CRETA] Turno #{appt.id} CONFIRMADO",
            "Tu turno en CRETA fue confirmado",
        ),
        "cancelled": (
            f"[CRETA] Turno #{appt.id} cancelado",
...
def seed_services():
    ...
    services = [
        Service(
            ...
            description="Estimula el colágeno y reafirma la piel.",
            duration_minutes=60,
            icon="✦",
        ),
        Service(
            name="Electrofitness Pasivo",
            description="Tonifica y modela mediante estimulación muscular.",
            duration_minutes=45,
            icon="⚡",
        ),
        Service(
            name="Depilación Láser",
            description="Eliminación definitiva del vello con láser.",
            duration_minutes=30,
            icon="◈",
        ),
        Service(
            name="Eliminación de Adiposidad Localizada",
            description="Ultrasonido focalizado no invasivo.",
            duration_minutes=50,
            icon="◉",
        ),
        Service(
            name="Eliminación de Flacidez",
            description="Radiofrecuencia que restaura la firmeza.",
            duration_minutes=60,
            icon="◇",
        ),
    ]
...
def book_appointment():
    ...
    try:
        ...
        return jsonify({"error": "Horario inválido."}), 422
...
def admin_login():
    ...
    if "bloqueado" in err.lower() or "demasiados intentos" in err.lower():
...
def admin_stats():
    ...
    confirmed_today = Appointment.query.filter(
        Appointment.appt_date == today,
        Appointment.status == AppointmentStatus.CONFIRMED,
    ).count()

    pending_all = Appointment.query.filter(
        Appointment.status == AppointmentStatus.PENDING
    ).count()
